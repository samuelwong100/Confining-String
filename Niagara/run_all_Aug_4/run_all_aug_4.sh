...
timeout 58m parallel --joblog "$SLURM_JOBID.log" -j $SLURM_TASKS_PER_NODE "python trial_run.py {1} {2} {3}" ::: {2..9} ::: {2..4} ::: $(seq 50 -5 5)
EXIT=$?

if [ $EXIT -eq 124 ]
then
    ssh -t nia-login01 "cd $SLURM_SUBMIT_DIR; sbatch trial_run_script.sh"
fi